...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/13 21:42:23 by foulare           #+#    #+#             */
/*   Updated: 2021/12/18 01:27:36 by foulare          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_init(int x, int y, float *z, t_info *info)
{
	...
	pmin = info->pmin;
	pmax = info->pmax;
	z[0] = ((double)x * (pmax.x - pmin.x)) / (double)WIDTH + pmin.x;
	z[1] = ((double)y * (pmax.y - pmin.y)) / (double)HEIGHT + pmin.y;
}
